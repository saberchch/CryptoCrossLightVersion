...
export default function DashboardRightPanel({
  ...
  return (
    <aside
      className={`dashboard-right-panel flex flex-col bg-gray-900 text-white shadow-md transition-all duration-300 ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Collapse Toggle */}
      ...
      <div className="flex-1 flex flex-col overflow-y-auto p-4">
        {!collapsed && (
          <>
            {/* Wallet */}
            <div className="mb-6 p-3 bg-gray-800 rounded-md shadow-sm">
              <h3 className="text-lg font-bold mb-1">Wallet Balance</h3>
              <p className="text-xl font-semibold">{walletBalance}</p>
            </div>

            {/* Subscription */}
            <div className="mb-6 p-3 bg-gray-800 rounded-md shadow-sm flex items-center gap-2">
              <svg
                fill="currentColor"
                height="28px"
                width="28px"
              ...
              <div className="flex flex-col">
                ...
                <p className="text-sm text-gray-300">{subscriptionPlan}</p>
              </div>
            </div>

            {/* Quick Actions */}
            <div className="flex flex-col gap-2 mt-auto">
              <button className="bg-yellow-500 text-black py-2 rounded-md hover:opacity-90">
                Start Quiz
              </button>
              <button className="bg-gray-700 text-white py-2 rounded-md hover:bg-gray-600">
                View Certificates
              </button>
            </div>
          </>
